...
function SBCApproval(props) {
    ...
    const loadPending = async () => {
        setIsLoading(true);
        const { success, data } = await api.post("staff/assessment/moderation/pending", { approvalLevel: 'SBC' });
    ...
    const approveModeration = async (mod) => {
        ...
        const { success, data } = await api.post("staff/assessment/moderation/approve", {
            ...
            moderationType: 'COLLEGE',
            approvedBy: props.loginData.StaffID
        });

        if (success && data.message === 'success') {
            toast.success(`SBC approved moderation for ${mod.ModuleCode}. ${data.approved} students updated.`);
            loadPending();
    ...
    return isLoading ? <Loader /> : (
        <div className="d-flex flex-column flex-row-fluid">
            <PageHeader title="SBC Approval" items={["Assessment", "Moderation", "SBC Approval"]} />
            <div className="flex-column-fluid">
                {/* Warning Banner */}
                <div className="alert alert-warning mb-4">
                    <i className="fa fa-exclamation-triangle me-2"></i>
                    <strong>SBC Final Approval:</strong> Approving these moderations will immediately update student records. This action cannot be undone.
                </div>

                <div className="d-flex justify-content-between align-items-center mb-4">
                    <h5 className="mb-0"><i className="fa fa-gavel me-2"></i>Pending College Moderations</h5>
                    <span className="badge bg-danger fs-6">{pendingModerations.length} Pending</span>
                </div>

                {pendingModerations.length === 0 ? (
                    <div className="alert alert-success text-center">
                        <i className="fa fa-check-circle me-2"></i>No pending moderations for SBC approval
                    </div>
                ) : (
                                    ...
                                    <tr key={index}>
                                        ...
                                        <td className="text-center">
                                            <button className="btn btn-success btn-sm me-1" onClick={() => approveModeration(mod)} disabled={processing === mod.ModuleCode}>
                                                {processing === mod.ModuleCode ? <span className="spinner-border spinner-border-sm"></span> : <><i className="fa fa-gavel me-1"></i>Approve</>}
                                            </button>
...
export default connect(mapStateToProps, null)(SBCApproval);
