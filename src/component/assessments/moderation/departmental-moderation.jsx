...
function DepartmentalModeration(props) {
    ...
    const applyModeration = async () => {
        ...
        const { success, data } = await api.post("staff/assessment/moderation/apply", {
            moduleCode: selectedModule.value, semester: selectedSemester.value, moderationType: "DEPARTMENTAL", mode,
        ...
        if (success && data.message === 'success') {
            toast.success(`Moderation applied to ${data.studentsModerated} students. ${data.requiresApproval ? 'Awaiting Dean approval.' : ''}`);
            loadStudents();
    ...
    return isLoading ? <Loader /> : (
        <div className="d-flex flex-column flex-row-fluid">
            <PageHeader title="Departmental Moderation" items={["Assessment", "Moderation", "Departmental"]} />
            <div className="flex-column-fluid">
                ...
                <div className="row g-3 mb-4">
                    ...
                    <div className="col-md-3 d-flex align-items-end">
                        <button className="btn btn-primary w-100" onClick={loadStudents} disabled={loadingStudents} style={{ height: '44px' }}>
                            ...
                            <div className="card-body">
                                ...
                                <div className="row g-3">
                                    ...
                                    <div className="col-md-3">
                                        ...
                                        <input type="number" className="form-control mt-2" value={adjustmentValue} onChange={(e) => setAdjustmentValue(e.target.value)} min="0" max="100" placeholder={adjustment === 'ADD_POINTS' ? 'Points to add' : 'New score'} />
                                    </div>
                                    <div className="col-md-4">
                                ...
                                <div className="mt-3 d-flex justify-content-between align-items-center">
                                    <div className="text-muted">
                                        ...
                                        {filteredStudents.length} student(s) will be moderated
                                        {adjustment === 'ADD_POINTS' ? ` (+${adjustmentValue} points)` : ` (set to ${adjustmentValue})`}
                                    </div>
                                    <button className="btn btn-success px-4" onClick={applyModeration} disabled={applying || filteredStudents.length === 0}>
                                        {applying ? <span className="spinner-border spinner-border-sm me-2"></span> : <i className="fa fa-check me-2"></i>}Apply Moderation
                                    </button>
                                </div>
                            </div>
                        </div>
                            ...
                            <table className="table table-hover table-striped">
                                ...
                                <tbody>
                                    {students.map((student, index) => {
                                        ...
                                        return (
                                            <tr key={student.EntryID} className={isFiltered ? 'table-warning' : ''}>
...
export default connect(mapStateToProps, null)(DepartmentalModeration);
