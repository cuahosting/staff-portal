...
function AcademicResultByStudent(props) {
    ...
    const getDepartmentStudents = async () => {
        // Endpoint for getting department students - currently unused in display
        await api.get(`staff/assessment/exam/result/students/${depart}`);
        setIsLoading(false);
    };

    const getData = async (semesterCode, department) => {
        setIsLoading(true);
        const { success, data: result } = await api.get(`staff/assessment/exam/result/by/department/${semesterCode}/${department}`);
        if (success && result?.length > 0) {
            let rows = [];
    ...
    const onSemesterChange = async (e) => {
        if (e.value !== "") { setSemeter({ ...semester, SemesterCode: e.value, SemesterCode2: e }); getData(e.value, depart); }
        else { setSemeter({ ...semester, SemesterCode: "", SemesterCode2: "" }); setData([]); }
    };

    useEffect(() => { getDepartmentStudents(); getSemesters(); }, []);

    return isLoading ? (<Loader />) : (
        <div className="d-flex flex-column flex-row-fluid">
            <PageHeader title={"ACADEMIC RESULT BY DEPARTMENT"} items={["Assessment", "ACADEMIC RESULT", " BY DEPARTMENT"]} />
            ...
            <div className="flex-column-fluid mb-2"><div className="row"><div className="mt-4">{data.length > 0 && <AgReportTable columns={columns} data={data} title={"ACADEMIC RESULT BY DEPARTMENT"} />}</div></div></div>
        </div>
    );
}
