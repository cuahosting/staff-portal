...
function CAEntry(props) {
    ...
    const findStudentsRegisteredModules = async (e) => {
        ...
        try {
            ...
            if (success && data) {
                ...
                if (data.student_list?.length > 0) {
                    data.student_list.forEach((item, index) => {
                        let ca_score_field = <input type="number" step={0.01} className="form-control" placeholder="Enter the score and press enter" max={ca_marked} onKeyDown={markCA} onChange={handleMarkScore} student_id={item.StudentID} mark_score={ca_marked} settings_id={selectedCAFields.SettingsID} id={'score_' + index} />;
                        if (data.ca_score?.length > 0) {
                            const student_score = data.ca_score.filter(i => i.StudentID === item.StudentID);
                            if (student_score.length > 0) { ca_score_field = <input type="number" step={0.01} className="form-control" defaultValue={student_score[0].CAScore} max={ca_marked} placeholder="Enter the score and press enter" onKeyDown={markCA} onChange={handleMarkScore} student_id={item.StudentID} mark_score={ca_marked} settings_id={selectedCAFields.SettingsID} id={'score_' + index} />; }
                        }
            ...
            setIsLoading(false);
        } catch (e) { setStudents([]); setIsLoading(false); }
    ...
    const onUploadCA = async () => {
        ...
        try {
            ...
            if (success) {
                ...
                if (message === 'success') { findStudentsRegisteredModules({ target: { value: createFindCARecord.SettingsID } }); toast.success('CA Score added successfully'); showAlert('Success', `CA Uploaded successfully, ${data.unregistered_students.length > 0 ? `However the following students did not registered for this course \n ${data.unregistered_students.join("\n")}.` : ''} \n ${data.over_scored.length > 0 ? `The following students has scores more than maximum score \n ${data.over_scored.join("\n")}.` : ''} `, `success`); }
