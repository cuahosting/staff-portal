...
function SenateUniversity(props) {
    ...
    const loadSemesters = async () => {
        ...
        if (success && data?.length > 0) {
            setSemesters(data.map(s => ({ value: s.SemesterCode, label: `${s.SemesterName} (${s.SemesterCode})` })));
        }
    };

    const loadReport = async (semester) => {
        if (!semester) return;
        setIsLoading(true);
        const { success, data } = await api.get(`staff/assessment/senate-reports/by-university/${semester}`);
        if (success && data?.modules) {
            setReportData(data.modules);
            setGradeColumns(data.gradeSettings || ['A', 'B', 'C', 'D', 'E', 'F']);
        } else {
            toast.info("No data found for this semester");
            setReportData(null);
        }
        setIsLoading(false);
    };
    ...
    const handleSemesterChange = (option) => {
        ...
        if (option) loadReport(option.value);
    };
    ...
    return (
        <div className="d-flex flex-column flex-row-fluid">
            <PageHeader
                title="Senate Report - University"
                items={["Assessment", "Senate Reports", "University View"]}
            />
                        ...
                        <div className="row">
                            ...
                            {reportData && (
                                <div className="col-md-4 d-flex align-items-end">
                                    <ReactToPrint
                                        trigger={() => <button className="btn btn-primary"><i className="fa fa-print me-2"></i>Print Report</button>}
                                        content={() => printRef.current}
                                        documentTitle={`Senate Report - University - ${selectedSemester?.label}`}
                                    />
                                </div>
                            )}
                        </div>
                ...
                {isLoading ? <Loader /> : (
                    <div ref={printRef} className="senate-report-container">
                        ...
                        <div className="senate-print-header">
                            ...
                            <h2>{schoolName}</h2>
                            <h3>Senate Result Report - University View</h3>
                        ...
                        {reportData && reportData.length > 0 ? (
                            ...
                            <div className="alert alert-info text-center">
                                <i className="fa fa-info-circle me-2"></i>No results found for the selected semester.
                            </div>
                        )}
                    </div>
                )}
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-scrollable">
                            <div className="modal-content">
                                ...
                                <div className="modal-body">
                                    {moduleDetails ? (
                                        <table className="table table-striped table-hover senate-modal-table">
                                            ...
                                            <tbody>
                                                {moduleDetails.students?.map((s, i) => (
                                                    <tr key={i}>
                                                        ...
                                                        <td><span className={`badge bg-${s.StudentGrade === 'A' ? 'success' : s.StudentGrade === 'F' ? 'danger' : 'secondary'}`}>{s.StudentGrade}</span></td>
                                                        <td>{s.Decision}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                ...
                                <div className="modal-footer">
                                    <button className="btn btn-secondary" onClick={() => { setShowModal(false); setModuleDetails(null); }}>Close</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default connect(mapStateToProps, null)(SenateUniversity);
