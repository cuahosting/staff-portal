...
function SenateDepartment(props) {
    ...
    const loadFilters = async () => {
        const [semRes, deptRes] = await Promise.all([
            ...
            api.get("staff/assessment/senate-reports/departments")
        ]);
        if (semRes.success) setSemesters(semRes.data.map(s => ({ value: s.SemesterCode, label: `${s.SemesterName} (${s.SemesterCode})` })));
        if (deptRes.success) setDepartments(deptRes.data.data.map(d => ({ value: d.DepartmentCode, label: d.DepartmentName })));
    };

    const loadReport = async () => {
        if (!selectedSemester || !selectedDepartment) return;
        setIsLoading(true);
        const { success, data } = await api.get(`staff/assessment/senate-reports/by-department/${selectedSemester.value}/${selectedDepartment.value}`);
    ...
    useEffect(() => { if (selectedSemester && selectedDepartment) loadReport(); }, [selectedSemester, selectedDepartment]);
    ...
    return (
        <div className="d-flex flex-column flex-row-fluid">
            <PageHeader title="Senate Report - Department" items={["Assessment", "Senate Reports", "Department View"]} />
                    ...
                    <div className="card-body">
                        <div className="row">
                            ...
                            <div className="col-md-3">
                                ...
                                <SearchSelect options={departments} value={selectedDepartment} onChange={setSelectedDepartment} placeholder="Select department..." />
                            </div>
                            {reportData && (
                ...
                {isLoading ? <Loader /> : (
                    <div ref={printRef} className="senate-report-container">
                        <div className="senate-print-header">
                            ...
                            <h2>{schoolName}</h2>
                            <h3>Senate Result Report - {selectedDepartment?.label}</h3>
...
export default connect(mapStateToProps, null)(SenateDepartment);
