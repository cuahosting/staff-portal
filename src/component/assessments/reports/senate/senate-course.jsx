...
function SenateCourse(props) {
    ...
    const loadFilters = async () => {
        const [semRes, courseRes] = await Promise.all([
            ...
            api.get("staff/assessment/senate-reports/courses")
        ]);
        if (semRes.success) setSemesters(semRes.data.map(s => ({ value: s.SemesterCode, label: `${s.SemesterName} (${s.SemesterCode})` })));
        if (courseRes.success) setCourses(courseRes.data.data.map(c => ({ value: c.CourseCode, label: c.CourseName })));
    };

    const loadReport = async () => {
        if (!selectedSemester || !selectedCourse) return;
        setIsLoading(true);
        const { success, data } = await api.get(`staff/assessment/senate-reports/by-course/${selectedSemester.value}/${selectedCourse.value}`);
    ...
    useEffect(() => { if (selectedSemester && selectedCourse) loadReport(); }, [selectedSemester, selectedCourse]);
    ...
    return (
        <div className="d-flex flex-column flex-row-fluid">
            <PageHeader title="Senate Report - Course" items={["Assessment", "Senate Reports", "Course View"]} />
                ...
                <div className="card mb-4 printPageButton">
                    <div className="card-body">
                        <div className="row">
                            ...
                            <div className="col-md-3">
                                <label className="form-label fw-semibold">Select Course/Program</label>
                                <SearchSelect options={courses} value={selectedCourse} onChange={setSelectedCourse} placeholder="Select course..." />
                            </div>
                            {reportData && (
                ...
                {isLoading ? <Loader /> : (
                    <div ref={printRef} className="senate-report-container">
                        <div className="senate-print-header">
                            ...
                            <h2>{schoolName}</h2>
                            <h3>Senate Result Report - {selectedCourse?.label}</h3>
                                ...
                                <div className="modal-body">
                                    {moduleDetails ? (
                                        <table className="table table-striped senate-modal-table">
                                            <thead className="table-dark"><tr><th>S/N</th><th>Student ID</th><th>Name</th><th>CA</th><th>Exam</th><th>Total</th><th>Grade</th><th>Decision</th></tr></thead>
                                            <tbody>
...
export default connect(mapStateToProps, null)(SenateCourse);
