...
function CASubmitted(props) {
    ...
    const getTimetable = async (semester) => {
        setIsLoading(true);
        const { success, data } = await api.get(`staff/assessment/exam/ca-submitted/${semester}`);
        if (success && data?.length > 0) {
            let rows = [];
    ...
    return isLoading ? (<Loader />) : (
        <div className="d-flex flex-column flex-row-fluid">
            <PageHeader title={"CA Submitted"} items={["Assessment", "Report", "CA Submitted"]} />
...
export default connect(mapStateToProps, null)(CASubmitted);
