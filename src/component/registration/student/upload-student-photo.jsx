...
function UploadStudentPhoto(props) {
    ...
    const getStudentList = async () => {
        try {
            const response = await axios.get(
                `${serverLink}staff/users/student-manager/update/get-all-student`,
                token
            );
    ...
    const getStudentPhotos = async (studentId) => {
        try {
            const response = await axios.get(
                `${serverLink}staff/student-management/student/photos/${studentId}`,
                token
            );
            setExistingPhotos(response.data || {});
        } catch (err) {
            console.error("Error fetching student photos:", err);
            setExistingPhotos({});
        }
    };
    ...
    const handleDeleteExistingPhoto = async (direction) => {
        ...
        try {
            ...
            await axios.delete(
                `${serverLink}staff/student-management/student/photos/${selectedStudent}/${direction}`,
                token
            );
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await axios.post(
                `${serverLink}staff/student-management/student/photos/upload`,
                formData,
                {
                    headers: {
    ...
    return isLoading ? (
        ...
        <div className="d-flex flex-column flex-row-fluid">
            <PageHeader
                ...
                items={["Registration", "Student Management", "Upload Photos"]}
            />
            ...
            <div className="row mb-4">
                <div className="col-lg-6 col-md-8">
                    <label className="form-label fw-bold">
                        <i className="fa fa-graduation-cap me-1"></i> Select Student
                    </label>
                    <SearchSelect
                        id="StudentID"
                        value={studentList.find(op => op.value === selectedStudent) || null}
                        options={studentList}
                        onChange={(selected) => handleStudentSelect({ target: { value: selected?.value || '' } })}
            ...
            {!selectedStudent && (
                <div className="text-center py-5 text-muted">
                    <i className="fa fa-graduation-cap" style={{ fontSize: '4rem' }}></i>
                    <p className="mt-3">Please select a student to manage their photos</p>
                </div>
            )}
        </div>
    );
