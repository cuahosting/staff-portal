...
function RegistrationDashboard(props) {
  ...
  const [datatable, setDatatable] = useState({ columns: [{ label: "S/N", field: "sn" }, { label: "Name", field: "name" }, { label: "Phone Number", field: "phone" }, { label: "Email Address", field: "email" }, { label: "Application Type", field: "type" }, { label: "ApplicationID", field: "appID" }, { label: "Course", field: "course" }, { label: "Date Applied", field: "date" }, { label: "", field: "action" }], rows: [] });
  ...
  async function getApplicationList() {
    ...
    if (success && data) {
      ...
      setAppList(data);
      if (data.length > 0) {
        const rows = data.map((applicant, index) => ({ sn: index + 1, name: applicant.FirstName + " " + applicant.MiddleName + " " + applicant.Surname, phone: applicant.PhoneNumber, email: applicant.EmailAddress, type: applicant.ApplicationType, date: formatDate(applicant.InsertedDate), appID: applicant.AppID, course: applicant.CourseName, action: applicant.ApplicationType === "undergraduate" ? (<Link to={`/application-processing-ug/${applicant.EntryID}`} className="btn btn-sm btn-primary">View</Link>) : (<Link to={`/application-processing-pg/${applicant.EntryID}`} className="btn btn-sm btn-primary">View</Link>) }));
  ...
  const buildTableRows = (list) => list.map((applicant, index) => ({ sn: index + 1, name: applicant.FirstName + " " + applicant.MiddleName + " " + applicant.Surname, phone: applicant.PhoneNumber, email: applicant.EmailAddress, type: applicant.ApplicationType, date: formatDate(applicant.InsertedDate), appID: applicant.AppID, course: applicant.CourseName, action: parseInt(applicant.Status) === 0 ? (<>--</>) : (applicant.ApplicationType === "undergraduate" ? (<Link to={`/application-processing-ug/${applicant.EntryID}`} className="btn btn-sm btn-primary">View</Link>) : (<Link to={`/application-processing-pg/${applicant.EntryID}`} className="btn btn-sm btn-primary">View</Link>)) }));
