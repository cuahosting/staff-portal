...
function SemesterProgression(props) {
            ...
            if (data.reg_setting.length < 1) {
                ...
                if (data.student_list.length > 0) {
                    ...
                    if (progression_list.length > 0) {
                        const rows = progression_list.map((item, index) => [(index + 1), item.StudentID, item.StudentName, item.CourseName, item.StudentLevel, item.StudentSemester, item.TotalCarryOver]);
                        setData(rows);
                        setStudentList(progression_list);
                        setProgressionStep(data.steps);
                    } else { toast.info("No pending progression"); }
                } else { toast.info("No pending progression"); }
            }
            setIsLoading(false);
    ...
    const runProgression = async () => {
        ...
        studentList.map(async item => {
            ...
            if (current_step.length > 0) {
                ...
                const next_step = progressionStep.filter(i => i.CourseCode === item.CourseCode && i.Step === (student_step + 1));
                if (next_step.length > 0) {
                    const sendData = { student_id: item.StudentID, student_level: next_step[0].CourseLevel, student_semester: next_step[0].CourseSemester, old_student_level: item.StudentLevel, old_student_semester: item.StudentSemester, semester_code: regSetting.active_semester, carry_over: item.TotalCarryOver, inserted_by: props.loginData.StaffID };
                    const { success, data } = await api.post("staff/registration/progression/progress-progression", sendData);
                    if (success && data?.message === 'success') toast.success(`${item.StudentID} progression successful`);
                    else if (success) toast.error(`Something went wrong progressing ${item.StudentID}. Please try again!`);
                } else { toast.error(`${item.StudentID} next level and semester progression step was not found. Please add from progression step`); }
            } else { toast.error(`${item.CourseName} current level and semester progression step was not found. Please add from progression step`); }
            counter_score += 1;
    ...
    useEffect(() => {
        if (counter === studentList.length && studentList.length > 0) {
            setTimeout(() => { toast.success("PROGRESSION RUN SUCCESSFUL"); setCounter(0); getRecords(); document.getElementById('run-progression').removeAttribute("disabled"); }, 3000);
        }
    }, [counter]);
            ...
            <div className="flex-column-fluid">
                <div className="card card-no-border">
                    <div className="card-body">
                        <div className="row">
                            <div className="col-md-4 Remaining"><b style={{ fontSize: '150px' }}>{counter}</b><hr /><p>Processed Progression</p></div>
                            <div className="col-md-4 Processed text-center text-uppercase"><b style={{ fontSize: '150px' }}>Of</b><h3 className="student_name"></h3><h3 className="percentage"></h3></div>
                            <div className="col-md-4 text-center"><b style={{ fontSize: '150px' }}>{studentList.length}</b><hr /><p>Total Active Not Progressed</p></div>
                        </div>
                        ...
                        <div className="row"><ReportTable data={data} columns={columns} height={"700px"} /></div>
                    </div>
                </div>
            </div>
        </div>
