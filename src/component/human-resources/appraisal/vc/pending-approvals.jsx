...
function VcPendingApprovals(props) {
    ...
    const loadData = async () => {
        setIsLoading(true);
        const { success, data } = await api.get("staff/hr/appraisal/vc/pending-appraisals");
        if (success) {
    ...
    return isLoading ? <Loader /> : (
        <div className="d-flex flex-column flex-row-fluid">
            <PageHeader title="Pending VC Approvals" items={["Human Resources", "Appraisal", "VC Approvals"]} />
                                            ...
                                            <tr key={appraisal.EntryID}>
                                                ...
                                                <td>
                                                    <span className="text-truncate d-inline-block" style={{ maxWidth: '120px' }}>
                                                        {appraisal.DeanRecommendation || '-'}
                                                    </span>
                                                </td>
                                                <td>
                                                    <span className="text-truncate d-inline-block" style={{ maxWidth: '120px' }}>
                                                        {appraisal.RegistrarRecommendation || '-'}
                                                    </span>
                                                </td>
                                                <td className="text-center">
                                                    <Link
                                                        to={`/human-resources/appraisal/vc/review/${appraisal.EntryID}`}
...
export default connect(mapStateToProps, null)(VcPendingApprovals);
