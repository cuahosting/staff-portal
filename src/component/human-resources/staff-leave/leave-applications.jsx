...
const StaffLeaveApplications = (props) => {
    ...
    const getData = async () => {
        const [catRes, appRes] = await Promise.all([
            api.get("staff/human-resources/staff-leave/leave-categories"),
            api.get(`staff/human-resources/staff-leave/leave-applications/${props.loginDetails[0].StaffID}`)
        ]);

        let leave_cat = [];
        if (catRes.success && catRes.data?.length > 0) {
            leave_cat = catRes.data;
            setLeaveCategory(catRes.data);
        }

        if (appRes.success && appRes.data?.length > 0) {
            const rows = appRes.data.map((item, index) => {
                const resultData = appRes.data;
                const casual = resultData.filter(x => x.LeaveType === 'Casual' && x.ApplicationStatus === 1);
                const annual = resultData.filter(x => x.LeaveType === 'Main' && x.ApplicationStatus === 1);
    ...
    const onEdit = (e) => {
        ...
        if (e.target.id === "EndDate" && leave.StartDate !== "") {
            ...
            const days_dif = time_diff / (1000 * 3600 * 24);
            const newDate = addDays(new Date(e.target.value), 1);
            setLeave({
                ...leave,
    ...
    const onSubmit = async (e) => {
        ...
        if (leave.LeaveType === "Main") {
            ...
            const no_days = parseInt(leave.AnnualDaysTaken) + parseInt(leave.DaysTaken);
        ...
        if (leave.LeaveType === "Casual") {
            ...
            const no_days = parseInt(leave.CasualDaysTaken) + parseInt(leave.DaysTaken);
    ...
    const applyLeave = async () => {
        const { success, data } = await api.patch("staff/human-resources/staff-leave/decide-leave", leave);
                                    ...
                                    <div className="d-flex flex-wrap">
                                        <div className="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6 mb-3">
                                            <div className="d-flex align-items-center">
                                                <div className="fs-2x fw-bold counted">{leave.LeaveDaysAllowed}</div>
                                            </div>
                                            <div className="fw-semibold fs-6 text-gray-600">Annual Days Allowed</div>
                                        </div>
                                        <div className="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6 mb-3">
                                            <div className="d-flex align-items-center">
                                                <div className="fs-2x fw-bold counted">{leave.AnnualDaysTaken}</div>
                                            </div>
                                            <div className="fw-semibold fs-6 text-gray-600">Annual Days Taken</div>
                                        </div>
                                        <div className="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6 mb-3">
                                            <div className="d-flex align-items-center">
                                                <div className="fs-2x fw-bold counted">{leave.CasualDaysAllowed}</div>
                                            </div>
                                            <div className="fw-semibold fs-6 text-gray-600">Casual Days Allowed</div>
                                        </div>
                                        <div className="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6 mb-3">
                                            <div className="d-flex align-items-center">
                                                <div className="fs-2x fw-bold counted">{leave.CasualDaysTaken}</div>
                                            </div>
