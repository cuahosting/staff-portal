    ...
    const handleChange = async () => {
        ...
        try {
            ...
            const { success, data: result } = await api.get(
                `staff/human-resources/finance-report/tuition-payment-report-date-range?startDate=${startDate}&endDate=${endDate}`
            );
            ...
            if (success) {
                if (result.data && result.data.length > 0) {
                    ...
                    toast.info("No tuition payments found for the selected criteria");
                    setCanSeeReport(false);
                    setData([]);
                }
            }
        } catch (err) {
            toast.error("Failed to fetch tuition payment report");
            console.error(err);
        } finally {
            setIsLoading(false);
        }
