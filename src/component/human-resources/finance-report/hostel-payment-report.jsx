...
function HostelPaymentReport(props) {
    ...
    const handleChange = async (semesterCode) => {
        ...
        try {
            ...
            const { success, data: result } = await api.get(
                `staff/human-resources/finance-report/hostel-payment-report?semester=${semesterCode}`
            );
            ...
            if (success) {
                if (result.data && result.data.length > 0) {
                    ...
                    toast.info("No hostel/accommodation payments found for this semester");
                    setCanSeeReport(false);
                    setData([]);
                }
            }
        } catch (err) {
            toast.error("Failed to fetch hostel payment report");
            console.error(err);
        } finally {
            setIsLoading(false);
        }
    ...
    return isLoading ? (
        ...
        <div className="d-flex flex-column flex-row-fluid">
            <PageHeader
                ...
                items={["Human Resources", "Finance & Records", "Hostel Payment Report"]}
            />
                            ...
                            <div className="row mt-5">
                                <div className="col-md-12">
                                    <div className="alert alert-info mb-3">
                                        <strong>Total Hostel/Accommodation Payments:</strong> {currencyConverter(totalAmount)} |
...
export default connect(mapStateToProps, null)(HostelPaymentReport);
