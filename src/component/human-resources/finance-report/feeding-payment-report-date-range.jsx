...
function FeedingPaymentReportDateRange(props) {
    ...
    const handleChange = async (semesterCode) => {
        ...
        try {
            ...
            const { success, data: result } = await api.get(
                `staff/human-resources/finance-report/feeding-payment-report-date-range?semester=${semesterCode}&startDate=${startDate}&endDate=${endDate}`
            );
    ...
    return isLoading ? (
        ...
        <div className="d-flex flex-column flex-row-fluid">
            <PageHeader
                ...
                items={["Human Resources", "Finance & Records", "Feeding Payment Report"]}
            />
                            ...
                            <div className="row mt-5">
                                <div className="col-md-12">
                                    <div className="alert alert-info mb-3">
                                        <strong>Total Feeding Payments:</strong> {currencyConverter(totalAmount)} |
                                        <strong> Total Records:</strong> {data.length}
                                    </div>
                                    <ReportTable
                                        title={`Feeding Payment Report - ${selectedSemester}`}
                                        columns={columns}
                                        data={data}
                                        height={tableHeight}
                                    />
...
export default connect(mapStateToProps, null)(FeedingPaymentReportDateRange);
