...
function PaymentByFaculty(props) {
    ...
    const [datatable, setDatatable] = useState({
        columns: [
            ...
            { label: "Collection Rate", field: "CollectionRate" },
        ],
        rows: [],
    });
    ...
    const getReportData = async () => {
        ...
        const result = await api.get(
            `staff/ac-finance/reports/payment-by-faculty?semester=${selectedSemester}`,
            token
        );
    ...
    const buildTable = (data) => {
        ...
        data.forEach((item, index) => {
            ...
            rows.push({
                sn: index + 1,
                Faculty: <span className="fw-bold">{item.FacultyName}</span>,
                ...
                CollectionRate: (
                    <div className="d-flex align-items-center">
                        <div className="progress flex-grow-1 me-2" style={{ height: "10px", width: "100px" }}>
                            <div
                        ...
                        <span className="fw-bold fs-6" style={{ minWidth: "50px" }}>{rate}%</span>
                    </div>
                ),
            });
        });
    ...
    const exportToCSV = () => {
        ...
        const headers = ["Faculty", "Departments", "Students", "Total Billed", "Total Paid", "Outstanding", "Collection Rate"];
        ...
        a.href = url;
        a.download = `payment_by_faculty_${selectedSemester}_${new Date().toISOString().split("T")[0]}.csv`;
        a.click();
    ...
    return isLoading ? (
        ...
        <div className="d-flex flex-column flex-row-fluid">
            <PageHeader
                title="Payment Report by Faculty"
                items={["Human Resources", "AC-Finance", "Reports", "By Faculty"]}
            />
                    ...
                    <>
                                        ...
                                        <div className="d-flex align-items-center">
                                            <div className="symbol symbol-50px me-3">
                                                <span className="symbol-label bg-primary">
                                                    <i className="fa fa-university text-white fs-3"></i>
                                                </span>
                                            </div>
                                            <div>
                                                <div className="fs-4 fw-bold">{summaryStats.totalFaculties}</div>
                                                <div className="text-muted small">Faculties</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="card">
                            <div className="card-header border-0 pt-5">
                                <h3 className="card-title">Payment Summary by Faculty</h3>
                            </div>
...
export default connect(mapStateToProps, null)(PaymentByFaculty);
