...
function SalarySummaryReport(props) {
    ...
    const getSalaryReport = async (salary_month) => {
        ...
        if (salary_month === "") {
            ...
            setIsLoading(true)
            await axios.get(`${serverLink}staff/human-resources/finance-report/journal/voucher?salary_month=${salary_month}`, token)
                .then((res) => {
                    console.log(res.data)
                    setReportData(res.data)
                    const allowanceItemTotals = res.data.allowance_list.reduce((acc, curr) => {
                    ...
                    const deductionItemTotals = res.data.deduction_list.reduce((acc, curr) => {
                        const itemName = curr.ItemName;
                        if (!acc[itemName]) {
                            acc[itemName] = { item_name: itemName, amount: 0 };
                        }
